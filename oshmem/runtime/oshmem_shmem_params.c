...
int oshmem_shmem_register_params(void)
{
    ...
    (void) mca_base_var_register("oshmem",
                                 ...
                                 NULL,
                                 "abort_delay",
                                "If nonzero, print out an identifying message when abort is invoked (hostname, PID of the process that called MPI_ABORT) and delay for that many seconds before exiting (a negative delay value means to never abort).  This allows attaching of a debugger before quitting the job.",
